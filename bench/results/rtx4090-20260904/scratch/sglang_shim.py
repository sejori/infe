...
class InfeDetector(BaseFormatDetector):
    ...
    def structure_info(self):
        """Required abstract method (constrained decoding for tool_choice). Mirrors HermesDetector."""
        from sglang.srt.function_call.core_types import StructureInfo
        if self._infe_dialect == "hermes":
            return lambda name: StructureInfo(begin='<tool_call>{"name":"' + name + '", "arguments":', end='}</tool_call>', trigger='<tool_call>')
        return lambda name: StructureInfo(begin='{"name":"' + name + '", "parameters":', end='}', trigger='')
