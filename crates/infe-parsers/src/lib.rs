...
//!
//! The streaming parser is called per engine step with decoded text chunks.
//! It produces structured deltas — [`ToolCallDelta`]s and
//! [`ReasoningDelta`]s — in the `OpenAI` streaming format.
//!
//! Each model dialect (Hermes, Llama-3 JSON, `DeepSeek`, …) is a
...
pub use types::{
    ParseError, ParserKind, ReasoningDelta, ToolCallArgState, ToolCallDelta, ToolCallState,
};
